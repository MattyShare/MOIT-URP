...
// basically we just need to add our 2 passes in there

namespace UnityEditor.Rendering.Universal.ShaderGraph
{
    sealed class UniversalMoitLitSubTarget : UniversalSubTarget/*, ILegacyTarget*/
    {
        static readonly GUID kSourceCodeGuid = new GUID("dfdf30a4f373b2940b6bcfa74fb8b6d6"); // GUID of this very .cs file, get it from the .meta file
        ...
        public UniversalMoitLitSubTarget()
        {
            displayName = "MOITLit";
        }

        // this should really be a dedicated ShaderID with relevant logic in ShaderUtils
        //protected override ShaderID shaderID => ShaderID.SG_Lit; // for now we'll use SG_Lit in case it sets up necessary inputs
        ...
        static class SubShaders
        {
            public static SubShaderDescriptor LitSubShader(UniversalTarget target, WorkflowMode workflowMode, string renderType, string renderQueue, string disableBatchingTag, bool complexLit, bool blendModePreserveSpecular)
            {
                ...
                // MOIT passes
                //result.passes.Add(LitPasses.GenerateMoments(target, CorePragmas.Instanced)); //Forward

                //result.passes.Add(PassVariant(LitPasses.GenerateMoments(target), CorePragmas.Instanced)); //Forward
                result.passes.Add(LitPasses.GenerateMoments(target, workflowMode, blendModePreserveSpecular, CorePragmas.Forward, LitKeywords.Forward));
                if (complexLit)
                {
                    result.passes.Add(LitPasses.ResolveMomentsComplex(target, workflowMode, complexLit, blendModePreserveSpecular, CoreBlockMasks.Vertex, LitBlockMasks.FragmentComplexLit, CorePragmas.Forward, LitKeywords.Forward));
                }
                else
                {
                    
                    result.passes.Add(LitPasses.ResolveMoments(target, workflowMode, blendModePreserveSpecular, CorePragmas.Forward, LitKeywords.Forward));
                }
        ...
        static class LitPasses
        {
            ...
            //moit
            static void AddMoitControlToPass(ref PassDescriptor pass, UniversalTarget target)
            {
                if (target.allowMaterialOverride)
                    pass.keywords.Add(LitKeywords.Moments);
                    pass.keywords.Add(LitKeywords.Trigonometric);
                    pass.keywords.Add(LitKeywords.MomentPrecision);
            }
            ...
            public static PassDescriptor GenerateMoments(
                UniversalTarget target/*,
                PragmaCollection pragmas*/)
            {
                var result = new PassDescriptor()
                {
                    // Definition
                    displayName = "GenerateMoments",
                    referenceName = "SHADERPASS_FORWARD",
                    //referenceName = "SHADERPASS_GENERATEMOMENTS",
                    ...
                    structs = CoreStructCollections.Default,
                    requiredFields = LitRequiredFields.Forward,
                    //requiredFields = LitRequiredFields.GenerateMoments,
                    fieldDependencies = CoreFieldDependencies.Default,
                    ...
                    //pragmas = pragmas ?? CorePragmas.Forward,     // NOTE: SM 2.0 only GL
                    pragmas = CorePragmas.Instanced,     // NOTE: SM 2.0 only GL
                    defines = new DefineCollection() { CoreDefines.UseFragmentFog },
                    //keywords = new KeywordCollection() { keywords },
                    keywords = new KeywordCollection() { LitKeywords.Moments, LitKeywords.Trigonometric, LitKeywords.MomentPrecision },
                    //includes = LitIncludes.Forward,
                    includes = LitIncludes.GenerateMoments,
                ...
                KeywordCollection keywords)
            {
                var result = new PassDescriptor()
                {
                    // Definition
                    displayName = "GenerateMoments",
                    referenceName = "SHADERPASS_FORWARD", // pretty sure we need this to be set up in the hlsl
                    //referenceName = "SHADERPASS_RESOLVEMOMENTS",
                    lightMode = "GenerateMoments",
                    //useInPreview = true,
                    ...
                    pragmas = pragmas ?? CorePragmas.Forward,     // NOTE: SM 2.0 only GL
                    defines = new DefineCollection() { CoreDefines.UseFragmentFog },
                    keywords = new KeywordCollection() { keywords, LitKeywords.Moments, LitKeywords.Trigonometric, LitKeywords.MomentPrecision },
                    //includes = LitIncludes.Forward,
                    includes = LitIncludes.GenerateMoments,
            ...
            public static PassDescriptor ResolveMoments(
                UniversalTarget target,
                WorkflowMode workflowMode,
                bool blendModePreserveSpecular,
                PragmaCollection pragmas,
                KeywordCollection keywords)
            {
                var result = new PassDescriptor()
                {
                    ...
                    keywords = new KeywordCollection() { keywords, LitKeywords.Moments, LitKeywords.Trigonometric, LitKeywords.MomentPrecision },
                    //includes = LitIncludes.Forward,
                    includes = LitIncludes.ResolveMoments,
            ...
            public static PassDescriptor ResolveMomentsComplex(
                UniversalTarget target,
                WorkflowMode workflowMode,
                bool complexLit,
                bool blendModePreserveSpecular,
                ...
                KeywordCollection keywords)
            {
                ...
                {
                    // Definition
                    displayName = "ResolveMomentsComplex",
                    //referenceName = "SHADERPASS_FORWARDONLY",
                    referenceName = "SHADERPASS_FORWARD",
                    //referenceName = "SHADERPASS_RESOLVEMOMENTS",
                    lightMode = "ResolveMoments",
                    ...
                    keywords = new KeywordCollection { keywords, LitKeywords.Moments, LitKeywords.Trigonometric, LitKeywords.MomentPrecision },
                    //includes = new IncludeCollection { LitIncludes.Forward },
                    includes = new IncludeCollection { LitIncludes.ResolveMoments },
        ...
        static class LitRequiredFields
        {
            ...
            public static readonly FieldCollection GenerateMoments = new FieldCollection()
            {
                StructFields.Attributes.uv1,
                //StructFields.Attributes.uv2,
                StructFields.Varyings.positionWS,
        ...
        static class LitKeywords
        {
            ...
            public static readonly KeywordDescriptor Moments = new KeywordDescriptor()
            {
                displayName = "MOIT Moments",
                referenceName = "",
                ...
                {
                    new KeywordEntry() { displayName = "4 Moments", referenceName = "_MOMENT4" },
                    new KeywordEntry() { displayName = "6 Moments", referenceName = "_MOMENT6" },
                    new KeywordEntry() { displayName = "8 Moments", referenceName = "_MOMENT8" },
                }
            };
            /*
            public static readonly KeywordDescriptor Trigonometric = new KeywordDescriptor()
            {
                displayName = "MOIT Trigonometric",
                referenceName = "",
                type = KeywordType.Enum,
                definition = KeywordDefinition.MultiCompile,
                scope = KeywordScope.Global,
                entries = new KeywordEntry[]
                {
                    ...
                    new KeywordEntry() { displayName = "On", referenceName = "_TRIGONOMETRIC" },
                }
            };
            */
            public static readonly KeywordDescriptor Trigonometric = new KeywordDescriptor()
            ...
            public static readonly KeywordDescriptor MomentPrecision = new KeywordDescriptor()
            {
                displayName = "MOIT Precision",
                referenceName = "",
                type = KeywordType.Enum,
                definition = KeywordDefinition.ShaderFeature,
                scope = KeywordScope.Global,
                ...
                entries = new KeywordEntry[]
                {
                    new KeywordEntry() { displayName = "Half", referenceName = "_MOMENT_HALF_PRECISION" },
                    new KeywordEntry() { displayName = "Single", referenceName = "_MOMENT_SINGLE_PRECISION" },
                }
            };
            //
        ...
        static class LitIncludes
        {
            ...
            const string kGenerateMomentsPass = "Assets/MOIT/ShaderLibrary/ShaderGraph/PBRForwardPassGenerateMoments.hlsl";
            const string kResolveMomentsPass = "Assets/MOIT/ShaderLibrary/ShaderGraph/PBRForwardPassResolveMoments.hlsl";
            ...
            public static readonly IncludeCollection GenerateMoments = new IncludeCollection
            {
                // Pre-graph
                { CoreIncludes.DOTSPregraph },
                { CoreIncludes.WriteRenderLayersPregraph },
                ...
                { CoreIncludes.CorePostgraph },
                //{ kForwardPass, IncludeLocation.Postgraph },
                { kGenerateMomentsPass, IncludeLocation.Postgraph },
            };

            public static readonly IncludeCollection ResolveMoments = new IncludeCollection
            {
                // Pre-graph
                { CoreIncludes.DOTSPregraph },
                { CoreIncludes.WriteRenderLayersPregraph },
                ...
                { CoreIncludes.CorePostgraph },
                //{ kForwardPass, IncludeLocation.Postgraph },
                { kResolveMomentsPass, IncludeLocation.Postgraph },
            };
        }
        #endregion
    }
    /*
    public static class MoitLitProperty
    {
